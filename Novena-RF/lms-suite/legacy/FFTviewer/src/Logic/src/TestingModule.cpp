...
{
	// disable data receiving and wait a bit for other thread to exit
	readingData = false;
	void *status;
...
void TestingModule::ReadData()
{
	...
	while (readingData)
	{
		...
		if (t2_info - t1_info > 1000) // each second display info and update data rate
		{
			...
			m_bytesPerSecond = m_BytesXferred / deltaT; // / 1000;
			m_BytesXferred = 0;
...
void TestingModule::ReadData_DigiRed()
{
	...
	while (readingData)
	{
		...
		if (rLen >= len) //start splitting buffer into IQ samples
		{
            ...
            {
                if( ((m_Buffers[i][b] & 0x10)>>4) == iq_select)
                    iq_select = !iq_select;
                else
                {
                    //iq_select = !(m_Buffers[i][b] & 0x10);
                    iq_select = !iq_select;
                    ++iq_select_errors;
                }
            }
            ...
            for(int b=1; b<len; b+=2)
            {
                if( ((m_Buffers[i][b] & 0x10)>>4) == iq_select_secondary)
                    iq_select_secondary = !iq_select_secondary;
                else
                {
                    iq_select_secondary = !(m_Buffers[i][b] & 0x10);
                    ++iq_select_errors_secondary;
                }
            }*/
		...
		if (t2_info - t1_info > 1000) // each second display info and update data rate
		{
			...
			cout << "iq select errors    = " << iq_select_errors << endl;
			cout << "iq secondary errors = " << iq_select_errors_secondary << endl;
