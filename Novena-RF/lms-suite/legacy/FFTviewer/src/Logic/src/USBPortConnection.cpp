            ...
            else //digired, controls through control end point
            {
                ...
                for (int i=0; i<USBDevicePrimary->EndPointCount(); i++)
                    if(USBDevicePrimary->EndPoints[i]->Address == 0x81)
                    {
                        ...
                        InEndPt = USBDevicePrimary->EndPoints[i];
                        InEndPt->MaxPktSize = InEndPt->MaxPktSize;
                        long len = InEndPt->MaxPktSize * 64;
                        // Each xfer request will get PPX isoc packets
                        InEndPt->SetXferSize(len);
                        break;
                ...
                /*if(devIsTx)
                {
                    ...
                    USBDeviceSecondary = new CCyUSBDevice();
                    for(int i=0; i< USBDeviceSecondary->DeviceCount(); i++)
                    {
                        USBDeviceSecondary->Open(i);
                        CCyControlEndPoint *mInCtrlEndPt3 = new CCyControlEndPoint(*USBDeviceSecondary->ControlEndPt);
                        CCyControlEndPoint *mOutCtrlEndPt3 = new CCyControlEndPoint(*USBDeviceSecondary->ControlEndPt);

                        mInCtrlEndPt3->ReqCode = CTR_R_REQCODE;
                        mInCtrlEndPt3->Value = CTR_R_VALUE;
                        mInCtrlEndPt3->Index = CTR_R_INDEX;

                        mOutCtrlEndPt3->ReqCode = CTR_W_REQCODE;
                        mOutCtrlEndPt3->Value = CTR_W_VALUE;
                        mOutCtrlEndPt3->Index = CTR_W_INDEX;
                        ...
                        memset(tbuf, 0, len);
                        tbuf[0] = CMD_GET_INFO;
                        mOutCtrlEndPt3->Write(tbuf, len);
                        mOutCtrlEndPt3->Read(tbuf, len);
                        ...
                        for (int i=0; i<USBDeviceSecondary->EndPointCount() && devIsRx; i++)
                            if(USBDeviceSecondary->EndPoints[i]->Address == 0x81)
                            {
                                InEndPt = USBDeviceSecondary->EndPoints[i];
                                InEndPt->MaxPktSize = InEndPt->MaxPktSize;
                                long len = InEndPt->MaxPktSize * 64;
                                // Each xfer request will get PPX isoc packets
                                InEndPt->SetXferSize(len);
                ...
                else
                {
                    ...
                    USBDeviceSecondary = USBDevicePrimary;

                    USBDevicePrimary = new CCyUSBDevice();
                    for(int i=0; i< USBDevicePrimary->DeviceCount(); i++)
                    {
                            ...
                            if(tbuf[0] == 1)
                                devIsTx = true;
                            for (int i=0; i<USBDevicePrimary->EndPointCount() && !devIsTx; i++)
                            if(USBDevicePrimary->EndPoints[i]->Address == 0x81)
                            {
                                InEndPt = USBDevicePrimary->EndPoints[i];
                                m_maxPacketSize = InEndPt->MaxPktSize;
...
//		dev_handle = libusb_open_device_with_vid_pid(ctx, 1204, 241);
        ...
        isConnected = true;
        //isoPckLen = libusb_get_max_iso_packet_size(libusb_get_device(dev_handle), 0x86);
        return true;
    }
    else
    {
...
/**
	@brief Reads data comming from the chip through USB port.
...
int USBPortConnection::FinishDataReading(unsigned char *buffer, long &length, int contextHandle)
{
    ...
    {
    #ifdef WIN32
	int status = InEndPt->FinishDataXfer(buffer, length, contexts[contextHandle].inOvLap, contexts[contextHandle].context);
...
int USBPortConnection::FinishDataSending(unsigned char *buffer, long &length, int contextHandle)
{
    ...
    {
    #ifdef WIN32
	int status = OutEndPt->FinishDataXfer(buffer, length, contextsToSend[contextHandle].inOvLap, contextsToSend[contextHandle].context);
