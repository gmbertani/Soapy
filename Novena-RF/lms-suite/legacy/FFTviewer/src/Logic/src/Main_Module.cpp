...
/**
	Creates thread that runs Windows message loop. It is needed by Connection manager
...
/*
void Main_Module::dumpAllRegisters()
{
    char outbuffer[60];
    int cmdCount = 0;
    char inbuffer[64];
    fstream out;
    out.open("Registers.txt", ios::out);
    ...
    getSerPort()->ReadData(outbuffer, 16, inbuffer, bytesToRead);
    for(int i=0; i<16; i++)
        out << (int)inbuffer[i] << endl;
    //Tx pll
    out << "[Tx PLL]" << endl;
