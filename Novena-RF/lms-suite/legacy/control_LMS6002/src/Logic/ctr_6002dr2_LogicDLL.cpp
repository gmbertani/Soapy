...
{
    return  getMainModule()->getSi5351C()->LoadRegValuesFromFile(FName);
}
/**
	@ingroup Board
...
{
    getMainModule()->getSi5351C()->ConfigureClocks();
...
{
    getMainModule()->getSi5351C()->Reset();
}

/**
    @ingroup Board
    @brief Sets selected clock to desired frequency
    @param clkID clock index number
    @param fOut_MHz clock output frequency
    @param enabled clock enabled
    @param inverted clock inverted
*/
...
/**
    @ingroup Board
    @brief Sets CLKIN frequency
    @param CLKIN_MHz input clock in MHz
*/
...
{
	int opened = getMainModule()->getSerPort()->OpenReceiver(i);
	if(opened)
		getMainModule()->UpdateVerRevMask();
	return opened;
}
...
{
	getMainModule()->getSerPort()->Close();
	return 1;
}
...
{
    getMainModule()->getSerPort()->SetControlDevice((eDeviceType)Type);
    return 1;
}
...
{
	return  getMainModule()->getSerPort()->GetControlDevice();
}
...
{
	return  getMainModule()->getSerPort()->GetOpenedReceiver();
}
...
{
	return getMainModule()->getSerPort()->GetReceiverList().size();
}
...
{
    vector<string> devices = getMainModule()->getSerPort()->GetReceiverList();
    if(index < devices.size() && index >= 0)
    {
        memcpy(name, devices[index].c_str(), strlen(devices[index].c_str())+1);
    }
}
